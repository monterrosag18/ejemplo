/* 
    ===================================
    DASHBOARD JAVASCRIPT - CRUDZASO v1
    ===================================
    
    ¡Bienvenido al cerebro del dashboard! 🧠✨
    
    Este es el archivo más complejo y emocionante de toda la aplicación.
    Aquí es donde los usuarios ven el panorama completo de su vida académica.
    
    El dashboard es como el "centro de control" de la nave espacial académica:
    🚀 Muestra métricas de rendimiento en tiempo real
    📊 Actualiza estadísticas automáticamente
    📋 Lista las tareas más importantes
    🔍 Permite navegación rápida a otras secciones
    ⚡ Proporciona acciones rápidas (crear, editar, completar tareas)
    
    Filosofía del dashboard:
    "En una sola mirada, el estudiante debe entender su situación académica actual
    y saber exactamente qué hacer a continuación para tener éxito."
    
    ¡Vamos a crear el dashboard más útil e inspirador del mundo! 🌟
*/
...
async function loadCurrentUser() {
    ...
    try {
        // Llamada a JSON Server para obtener datos actualizados del usuario
        const response = await fetch(`${DASHBOARD_CONFIG.API_BASE_URL}/users/${sessionData.userId}`);
        ...
        console.log(`👤 Current user loaded from API: ${dashboardState.currentUser.name}`);
        
        // Actualizar la interfaz con la información del usuario
        updateUserInterface();
        
    } catch (error) {
        console.error('⚠️ Error loading user data, using session fallback:', error);
...
async function loadUserTasks() {
    console.log('📋 Loading user tasks from JSON Server...');
    
    try {
        // Llamada real a JSON Server API para obtener tareas del usuario
        const response = await fetch(`${DASHBOARD_CONFIG.API_BASE_URL}/tasks?userId=${dashboardState.currentUser.id}`);
        ...
        // Guardar tareas en el estado del dashboard
        dashboardState.tasks = tasks;
        dashboardState.filteredTasks = [...tasks];
        
        console.log(`✅ Loaded ${dashboardState.tasks.length} tasks from API`);
        
    } catch (error) {
        console.error('⚠️ Error loading tasks from API, using fallback:', error);
        ...
        try {
            ...
            console.log(`📦 Loaded ${dashboardState.tasks.length} tasks from localStorage fallback`);
